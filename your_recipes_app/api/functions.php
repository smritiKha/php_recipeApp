	...
	public function getSettings() {
		...
		if (isset($_GET['api_key'])) {
			...
			if ($access_key_received == $api_key) {
				...
				$respon = array('status' => 'ok', 'post' => $result);
	...
	public function getConfig() {
		...
		if (isset($_GET['api_key'])) {
			...
			if ($access_key_received == $api_key) {
				...
				$sqlPlacements = "SELECT * FROM tbl_ads_placement WHERE ads_placement_id = 1";

				$settings = $this->get_one($sqlSettings);
				$ads = $this->get_one($sqlAds);
				$adsPlacement = $this->get_one($sqlPlacements);
				...
				if ($appCount > 0) {
					$respon = array('status' => 'ok', 'app' => $app, 'ads' => $ads, 'ads_placement' => $adsPlacement, 'settings' => $settings);
				} else {
					$respon = array('status' => 'ok', 'app' => $AppArray, 'ads' => $ads, 'ads_placement' => $adsPlacement, 'settings' => $settings);
				}
